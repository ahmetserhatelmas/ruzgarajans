...
import { revokeApplicationShareAction } from "@/lib/actions";
import { Button } from "@/components/ui/button";
import { shareApplicationIds, sharePinLabel, shareRemainingLabel } from "@/lib/share-label";
import type { ApplicationShare } from "@/lib/types";

export function ShareApplicationPacks({
  shares,
  urls,
  names,
  highlightToken,
  ...
  return (
    <div className="space-y-3 rounded-xl bg-card p-4 ring-1 ring-foreground/10">
      <div>
        <p className="font-medium">Şifreli başvuru linkleri</p>
      ...
      <ul className="space-y-3">
        {shares.map((share) => {
          ...
          const ids = shareApplicationIds(share);
          const labels = ids.map((id) => names[id] || "Başvuru").filter(Boolean);
              ...
              <div className="flex flex-wrap items-start justify-between gap-2 text-sm">
                <div className="min-w-0">
                  <p className="font-medium">
                    {labels.length ? labels.slice(0, 4).join(", ") : "Paket"}
                    {labels.length > 4 ? ` +${labels.length - 4}` : ""}
                  </p>
                  <p className="text-muted-foreground">
                    {ids.length} başvuru · {shareRemainingLabel(share.expires_at)}
                    {pin ? ` · şifre ${pin}` : ""}
                  </p>
                </div>
                <form action={revokeApplicationShareAction}>
                  <input type="hidden" name="share_id" value={share.id} />
                  <input type="hidden" name="application_id" value={share.application_id} />
