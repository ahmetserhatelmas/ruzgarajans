...
function shareWhatsAppText(url: string, pin: string, title: string) {
  return `Başvuru dosyası:\n${title}\n${url}\n\nAçılış şifresi: ${pin}\n(Şifreyi linkin içine yazma, sayfada sorunca gir.)`;
}

export function ShareApplicationPanel({
  applicationId,
  title,
  shares,
  directors,
  ...
  return (
    <div className="space-y-3 rounded-xl border border-border bg-card p-4">
      <div>
        <p className="font-medium">Şifreli linkle paylaş</p>
        <p className="text-sm text-muted-foreground">
          4 haneli şifre ve süre koy. Karşı taraf giriş yapmaz, şifreyi yazar.
        </p>
        {pinError ? (
          <p className="mt-2 text-sm text-destructive">Link için 4 haneli şifre yaz.</p>
        ) : null}
      </div>
      <form action={createApplicationShareAction} className="grid gap-3">
      ...
      {shares.length ? (
        <ul className="space-y-3">
          {shares.map((share) => {
            ...
            const waText = savedPin ? shareWhatsAppText(url, savedPin, title) : url;
            return (
